...
var $window     = jQuery(window),           // Cached window
...
var global = {
    ...
    init: function(){
        ...
        // If form found, init it
        if ( jQuery('form.contact-form').length ) {
            forms.init();
        }

        // If slider found, init it
        if ( jQuery('.component--fullpage').length ) {
            homeSlides.init();
        }

    },
    widthIsMobile: function(){
        return jQuery(window).width() < settings.mobileWidth ? true : false;
    },
...
var menu = {
    init: function() {
        menu.hamburger();
        jQuery('.menu-main-menu-container').css( 'height', jQuery(window).height() + 'px' );

    },
    hamburger: function() {
        jQuery('#menu-expand-button').click(function(){
            jQuery(this).find('.menu-expand-button--icon').toggleClass('open');
            jQuery('.menu-main-menu-container').toggleClass('expand');
        });
    }
};
...
var homeSlides = {
    init: function() {
        ...
        jQuery('#component--fullpage--slider').fullpage({
            navigation: true,
            ...
            onLeave: function(index, nextIndex, direction){
                ...
                var $nextSection = jQuery( '#component--fullpage--slider .section:nth-child(' + nextIndex + ')' ),
                    $navigation = jQuery('#fp-nav');

                // Set text/navigation color
            ...
            afterLoad: function(anchorLink, index){
                ...
                var $loadedSection = jQuery( '#component--fullpage--slider .section:nth-child(' + index + ')' ),
                    $navigation = jQuery('#fp-nav');;

                if ( ! $loadedSection.hasClass('transparent') ) {
                    $navigation.addClass('dark');
                }
            },
    ...
    hideHint: function() {
        jQuery('.component--fullpage--hint-button').removeClass('show');
    },
    showHint: function() {
        jQuery('.component--fullpage--hint-button').addClass('show');
    }
};
...
var announcement = {
    init: function() {
        jQuery('.component-announcement--dismiss-button').on('click', function(){
            announcement.dismiss();
        });
    },
    dismiss: function() {
        jQuery('.component-announcement').addClass('dismiss');
    }
};
...
var forms = {
    init: function() {
        ...
        // init
        forms.radioToggles();
        forms.textAreaHeights();
        forms.validate();
        forms.labelsToPlaceholders();

        jQuery('input[id$="telephonenumber"], input[id$="faxnumberoptional"]').inputmask('Regex', { regex: "^[+()0-9 -]+$" });
    ...
    textAreaHeights: function() {

        // Set textarea depending on label height during page load
        jQuery('textarea').each(function( index ) {

            var height = jQuery(this).siblings('label').outerHeight();
        ...
        // Update textarea to 1 row when entering text
        jQuery('.contact-form textarea').on('focus', function() {

            var current_rows = jQuery(this).attr('rows');

            jQuery(this).attr('data-rows', current_rows );
            jQuery(this).attr('rows', 1);

        });

        // Reset original rows if user blurs without entering text
        jQuery('.contact-form textarea').on('blur', function() {

            if ( jQuery(this).val() == "" ) {

                var original_rows = jQuery(this).attr('data-rows');

                jQuery(this).attr('rows', original_rows );
    ...
    validate: function() {
        ...
        // Custom validation rule for emails
        ...
        jQuery(".contact-form").validate({
            ...
            highlight: function(element) {
                jQuery(element).parent('div').addClass('field-error');
            },
            unhighlight: function(element) {
                jQuery(element).parent('div').removeClass('field-error');
            },
            focusInvalid: false,
            invalidHandler: function(event, validator) {
                    ...
                    // Custom radio validation for jetpack contact-form
                    //
                    // jQuery validate doesnt work correctly with jetpack
                    // radio buttons so well add some custom validation
                    forms.validateRadios();

                    // Quick timeout to make sure everything is ready for this next step
                    setTimeout(function() {

                        // If a radio button is the first instance of a field error, then set it
                        // If the first instance is not a radio button, use jquery-validates default errorList array
                        if ( jQuery('.contact-form div.field-error').first().find('input[type="radio"]').length ) {
                             $errorElement = jQuery('.contact-form div.field-error').first();
                        } else {
                            $errorElement = jQuery(validator.errorList[0].element);
                        }

                        // Scroll to first instance of a field error
                        jQuery('html, body').animate({
                            scrollTop: $errorElement.offset().top -130
                        }, 700);
        ...
        jQuery('input[type="email"]').rules("add", {
            required:  true,
            customemail: true
        });
    },
    labelsToPlaceholders: function() {
        ...
        jQuery('.contact-form input[type="text"], .contact-form input[type="email"], .contact-form textarea').each( function(index) {

            var $this = jQuery(this),
                placeholder = $this.siblings('label').text().replace('(required)','');
    ...
    validateRadios: function() {
        ...
        // Custom radio button validation for jetpack contact-form
        var errors = false;
        var $radioGroups = jQuery( '.grunion-radio-label.radio' ).siblings('.grunion-field-label');

        if ( $radioGroups.length > 0 ) {

            $radioGroups.each( function( index ) {

                var numberOfCheckedRadios = jQuery(this).siblings('.active').length;

                if ( numberOfCheckedRadios <= 0 ) {

                    jQuery(this).addClass('field-error').css('display', 'inline-block');
                    jQuery(this).closest('div').addClass('field-error');
                    errors = true;

                } else {
                    jQuery(this).removeClass('field-error');
                    jQuery(this).closest('div').removeClass('field-error');
                }
