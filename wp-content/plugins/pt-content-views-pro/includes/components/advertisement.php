...
class CVP_ADVERT {
	...
	static function _enable_ads() {
		return PT_CV_Functions::setting_value( PT_CV_PREFIX . 'ads-enable' ) && !PT_CV_Functions::get_global_variable( 'reused_view' );
	}
	...
	static function count_ads() {
		if ( self::_enable_ads() ) {
			...
			return count( array_filter( $ads_content ) );
		}
	...
	static function modify_params( $args ) {
		if ( self::_enable_ads() ) {
			...
			if ( $apply ) {
				...
				$ads_content	 = (array) PT_CV_Functions::settings_values_by_prefix( PT_CV_PREFIX . 'ads-content' );
				$ads_count		 = count( array_filter( $ads_content ) );
