	...
	private function output_plugin_acf() {
		if ( function_exists( 'get_field_object' ) ) {
			if ( $field_object = get_field_object( $this->key, $this->object ) ) {
				$this->field_value	 = PT_CV_ACF::display_output( $field_object );
	...
	if ( $custom_fields_st && !empty( $custom_fields_st[ 'list' ] ) ) {
		...
		foreach ( $wanted_keys as $idx => $key ) {
			...
			if ( !empty( $ctf_date_format ) ) {
				$try_convert = PT_CV_Html_Pro::date_convert( $field_value, $ctf_date_format );
		...
		$html = sprintf( '<div class="%s" %s>%s</div>', PT_CV_PREFIX . 'ctf-list', $_data, implode( '', $result ) );
	}
