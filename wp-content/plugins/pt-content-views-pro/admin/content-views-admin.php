...
class PT_Content_Views_Pro_Admin extends PT_Content_Views_Admin {
			...
			PT_CV_Asset::localize_script(
				...
				'custom_field'			 => array(
					'type_operator' => array(
						...
						'DATE'		 => array( 'TODAY', 'NOW_FUTURE', 'IN_PAST', '=', '!=', '>', '>=', '<', '<=', 'BETWEEN', 'NOT BETWEEN', 'EXISTS', 'NOT EXISTS' ),
	...
	public function filter_setting_post_in( $args ) {
		$args = __( 'Enter post ID, or type to search by post Title', 'content-views-pro' ) . '. ' . __( 'Drag and drop to change display order of them', 'content-views-pro' );
		return $args;
	}

	// Add more description for Post_not_in
	public function filter_setting_post_not_in( $args ) {
		$args = __( 'Enter post ID, or type to search by post Title', 'content-views-pro' );
		return $args;
	}
							...
							array(
								...
								'params'		 => array(
									array(
										...
										'options'	 => array(
											''		 => __( 'Hard resize (generate new images, takes time and resources). Width or height can be empty.', 'content-views-pro' ),
			...
			array(
				...
				'params'	 => array(
					array(
						...
						'std'	 => '',
						'desc'	 => sprintf( __( 'By default, whole page will be loaded (Header, Content, Footer). To load only Content, please enter selector to idenfity it %s', 'content-views-pro' ), '(for example: <code>#content</code> or <code>.entry-content</code>)' ),
					),
				),
				'dependence' => array( $prefix . 'open-in', PT_CV_PREFIX . 'lightbox' ),
			),
									...
									'params' => array(
										array(
											...
											'params'		 => array(
												array(
													'type'		 => 'html',
													'content'	 => sprintf( '<p class="text-muted alert alert-warning" style="margin-top: -20px">%s.</p>', __( 'Notice: Normal pagination is not recommended for Shuffle Filter. Please use Ajax pagination for better result', 'content-views-pro' ) ),
												),
											),
						...
						array(
							...
							'params' => array(
								array(
									'type'		 => 'select',
									'name'		 => $prefix . '-list',
									'options'	 => PT_CV_Values_Pro::custom_fields( false, 'sort' ),
									'std'		 => '',
									'class'		 => 'select2-sortable',
									'multiple'	 => '1',
									'desc'		 => __( 'Field is selectable only if it was added to at least one post', 'content-views-pro' ) . '. ' . __( 'Drag & drop to change display order of fields', 'content-views-pro' ),
								),
							),
						),
						cvp_ctf_multi_plugins() ? array(
							...
							'params'		 => array(
								array(
									...
									'std'		 => '',
									'desc'		 => __( 'There are more than one custom field plugin are activated. Which plugin you used to create above fields?', 'content-views-pro' ),
								),
							),
							) : null,
						// Hide empty field
			...
			$options = array(
				array(
					...
					'params'		 => array(
						array(
							'type'			 => 'checkbox',
							'name'			 => 'enable-' . $prefix,
							'options'		 => PT_CV_Values::yes_no( 'yes', __( 'Show selected terms to filter posts in output', 'content-views-pro' ) ),
							'std'			 => '',
				...
				array(
					...
					'params'		 => array(
						array(
							'type'		 => 'html',
							'content'	 => sprintf( '<p class="text-muted" style="margin-top: -10px;font-size: 13px;">%s.</p>', sprintf( __( 'Work with %s layout', 'content-views-pro' ), '<code>' . __( 'Grid', 'content-views-query-and-display-post-page' ) . ', ' . __( 'Pinterest', 'content-views-pro' ) . ', ' . __( 'Masonry', 'content-views-pro' ) . ', ' . __( 'Collapsible List', 'content-views-query-and-display-post-page' ) . '</code>' ) ),
						),
					),
				),
				array(
												...
												array(
													...
													'params'		 => array(
														array(
															...
															'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Allow visitors to change this operator in output', 'content-views-pro' ) ),
															'std'		 => '',
														),
													),
												),
				...
				array(
					'label'			 => array(
						'text' => __( 'Cross operator', 'content-views-pro' ),
					),
					'extra_setting'	 => array(
						'params' => array(
					...
					'params'		 => array(
						array(
							...
							'std'		 => 'and',
							'desc'		 => __( 'The logical relationship between taxonomies', 'content-views-pro' )
						),
					),
					'dependence'	 => array( 'enable-' . $prefix, 'yes' ),
				),
								...
								array(
									...
									'params'		 => array(
										array(
											...
											'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Hide empty terms (which was not assigned to any posts)', 'content-views-pro' ) ),
											'std'		 => '',
										),
									),
								),
								array(
									...
									'params'		 => array(
										array(
											...
											'std'		 => '',
											'desc'		 => __( 'This value is got from table "term_taxonomy", and do not depend on filter settings of View', 'content-views-pro' ),
										),
									),
								),
								array(
