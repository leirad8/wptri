    ...
    public static function esig_access_control_sidebar() {
       ...
       // $api = new WP_E_Api();
        ...
        $content .= '
					...
					<label style="font-size:14px;"><b> ' . __('Select one (or multiple) USERS that will need to sign this document.', 'esig') . '</label>
        ...
        $content .= '<h4><span>' . __("Document Description","esig") . '</span></h4>';
        if ($document_id) {
            $access_control = json_decode(WP_E_Sig()->meta->get($document_id, 'esig_wpaccess_control'));
        }
