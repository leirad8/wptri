...
(function ($) {
    ...
    $("#addRecipient_temp").on("click", function (e) {
        ...
        $("#recipient_emails_temp").append('<div id="signer_main_temp">' +
                '<input type="text" name="recipient_fnames[]"  placeholder="Signers Name"  />' +
                '<input type="text" name="recipient_emails[]"  placeholder="email@address.com" style="width:213px;"  value="" />' + slv + '<span id="esig-del-signer" class="deleteIcon"></span></div>');
...
// email validation checking on basic document add view . 
    ...
    $.fn.template_email_duplicate = function () {
        ...
        for (var i = 0; i < view_fname.length; i++) {
            ...
            if (blank)
            {
                $('#standard_view_popup_bottom .af-inner .esig-error-box').remove();
                $('#standard_view_popup_bottom .af-inner').append('<span class="esig-error-box">*A Full name including your first and last name is required.</span>');
                return true;
            }
        }
        ...
        for (var i = 0; i < view_email.length; i++) {
            ...
            } else if (!esign.is_valid_email(view_email[i]))
            {
                ...
                $('#standard_view_popup_bottom .af-inner .esig-error-box').remove();
                // add new error msg 
                $('#standard_view_popup_bottom .af-inner').append('<span class="esig-error-box">*E-mail address is not valid</span>');
                return true;
            }
            if (blank_email)
            {
                ...
                $('#standard_view_popup_bottom .af-inner .esig-error-box').remove();
                // add new error msg 
                $('#standard_view_popup_bottom .af-inner').append('<span class="esig-error-box">*You must fill email address.</span>');
                return true;
            }
        }
        ...
        if (exists)
        {
            ...
            $('#standard_view_popup_bottom .af-inner').append('<span class="esig-error-box">*You can not use duplicate email address.</span>');
