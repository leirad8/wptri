...
class Dropbox_API {
    ...
    public function prepareFile($file,$filename) {
        
        if (function_exists('curl_file_create')) {
            $contentType = mime_content_type($file);
            return curl_file_create($file, $contentType, $file);
        }
        return '@'. str_replace('\\', '/', $file) . ';filename=' . $filename;
    }
    ...
    public function putFile($file, $filename = false, $path = '', $overwrite = true) {
        if (file_exists($file)) {
            if (filesize($file) <= 157286400) {
                ...
                $params = array(
                    'filename' => $filename,
                    'file' => $this->prepareFile($file, $filename),
