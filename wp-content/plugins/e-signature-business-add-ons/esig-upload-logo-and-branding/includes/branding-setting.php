...
class esigBrandingSetting {
    ...
    public function emailFooterText($wpUserId = false) {
        ...
        if ($emailFooterText) {
            return htmlspecialchars(stripslashes($emailFooterText));
        }
        return htmlspecialchars(stripslashes(WP_E_Sig()->setting->get_generic('esig_branding_email_footer_text' . $this->wpUserId($wpUserId))));
    }
