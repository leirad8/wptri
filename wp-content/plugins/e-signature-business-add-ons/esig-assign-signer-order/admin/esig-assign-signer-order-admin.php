    ...
    class ESIG_ASSIGN_ORDER_Admin extends ESIGN_SIGNER_ORDER_SETTING {
        ...
        public function load_signer_order_display($content,$readonly,$document_id,$index){
              ...
              if(!$readonly && self::is_signer_order_active($document_id)){
                  $content ='<span id="signer-sl" class="signer-sl">' . $sl . '.</span><span class="field_arrows"><span id="esig_signer_up"  class="up"> &nbsp; </span><span id="esig_signer_down"  class="down"> &nbsp; </span></span>' ;
              } 
              return $content;
        }
        ...
        public function show_signer_order_link($order_content, $document_id) {
            ...
            if ($invitation_count > 1) {
                $order_content = '<span id="esign-signer-order-show"><input type="checkbox" id="esign-assign-signer-order" name="esign_assign_signer_order" ' . $checked . ' value="1">' . __('Assign signer order', 'esig_order'). '</span>';
            } else {
               
                $order_content = '<span id="esign-signer-order-show" style="display:none;"><input type="checkbox" id="esign-assign-signer-order" name="esign_assign_signer_order" ' . $checked . ' value="1">' . __('Assign signer order', 'esig_order') . '</span>';
            }
